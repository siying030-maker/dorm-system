...
def get_floor_sheet_name(
    ...
    if dorm not in DORM_PREFIX:
        raise ValueError(
            f"找不到宿舍「{dorm}」的 Sheet 前綴設定"
        )

    prefix = DORM_PREFIX[dorm]
...
def get_login_dorm_options(term):
    ...
    if manage_dorms:
        ...
        # 只保留目前真的有設定 Sheet 的宿舍
        valid_dorms = [
            dorm
            for dorm in dorms
            if dorm in ATTENDANCE_SHEETS.get(
                "上學期",
                {}
            )
            or dorm in VACATION_SHEETS.get(
                term,
                {}
            )
        ]
